...
def generate_picture(width=120, height=26):
...
def draw_str(count, image, font_size):
    ...
    font_file = os.path.join('MSYH.ttf')
...
def noise(image, width=120, height=26, line_count=3, point_count=20):
    '''
...
def valid_code():
    ...
    valid_str, image = draw_str(4, image, 20)
    image = noise(image)
